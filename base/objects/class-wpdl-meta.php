...
if ( ! class_exists( 'WPDL_Meta' ) ) {
	 ...
	 */
	class WPDL_Meta implements WPDL_Object {
		private static $meta = [];
		 ...
		 * @return ?WPDL_Meta
		 */
		public static function factory( string $object_type, string $meta_key, string $object_subtype ): ?WPDL_Meta {
			global $wp_meta_keys;
			...
			if ( isset( $wp_meta_keys[ $object_type ][ $object_subtype ][ $meta_key ] ) ) {
				...
				if ( ! isset( static::$meta[ $object_type ][ $object_subtype ][ $meta_key ] ) ) {
					static::$meta[ $object_type ][ $object_subtype ][ $meta_key ] =
						new WPDL_Meta( $meta_key, $object_type, $args );
				}
